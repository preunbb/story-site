# Over Easy product: Luna Identity Arm™
...
- **model:** `OET-LIA-1`
- **image (target):** `../assets/scenes/andrea_and_lucas/overeasy_luna_identity_arm_device_v1.png`
- **badge:** Hardware
- **tagline:** Engineered for when hints aren't enough.
- **description:** Luna Identity Arm™ — Over Easy hardware for the woman who prefers a firm answer to awkward situations. Discreet enough to carry, decisive enough that he won't need a second explanation.
...
```
Product catalog photograph of Over Easy Technologies hardware — Luna Identity Arm™.
...
Save to: assets/scenes/andrea_and_lucas/overeasy_luna_identity_arm_device_v1.png
Use reference images when available. Never overwrite an existing file — this is a new versioned asset.
```

**Filename:** `overeasy_luna_identity_arm_device_v1.png`

## 4. Apply catalog entry

```bash
npm run oe-product -- "Luna Identity Arm™" --apply
```

Or merge `over-easy-products/drafts/luna-identity-arm/catalog-entry.json`.

## 5. Preview

http://localhost:8080/over-easy-products/#product-luna-identity-arm
