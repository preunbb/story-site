# Andrea and Lucas — testicle stats (Ch.1–20)

## Male testicle status

| Male character | Left testicle | Right testicle |
| --- | --- | --- |
| **Lucas** | **Damaged** — never popped through Ch.20. Ch.11 chunk stomp; Ch.16 Eve’s crescent rupture; Ch.18 Abby re-opens the slit; Ch.20 Kay carves a third slash across the two parallel vents (permanent **inequality-symbol** wound on his **left** nut). | **Damaged** — never popped through Ch.20. Bruised/denied throughout capture; Ch.11 stomp damage; Ch.14 Kay targets it as the “smaller” nut for future rites; Ch.20 Kay knee-crushes both while making the left-nut slash. |
| **Hunter** | **Popped** — Ch.3, Andrea: heel stomp/twist on his **left** ball until it collapses into pulp while she fucks Lucas on top of him. | **Popped** — Ch.3, Andrea: grinds his **remaining** testicle under her boots until it loses structural integrity (“neutered”). |
| **Nate** | **Popped (offscreen)** — ~8 months before the mall arc; Izzie pushes him crotch-first onto a stair handrail and grinds both nuts the full length (shown in Ch.6–7 flashback video). Doctors could not save either. | **Popped (offscreen)** — same rail grind; empty crotch by the time Lucas meets him at the jeans store. |
| **Peter** | **Popped (offscreen)** — before the story; Andrea’s prior eunuch lover (Ch.1). Both nuts gone; empty sack kept. | **Popped (offscreen)** — same; referenced only in Andrea’s Ch.1 anecdote. |
| **Greyson** | **Popped** — Ch.8–9, Andrea: back-heel halfstration ruptures his **left** nut on the dance floor; Tamara surgically removes the remains in Ch.9. Last seen Ch.9 as a uniballer. | **Healthy (last known)** — intact when Tamara releases him; he fears losing it but is not shown again after Ch.9. |
| **Sunni** | **Popped** — Ch.10, Yvette: botched “repair” ends in full castration after double rupture (concert damage + Yvette’s exam). | **Popped** — Ch.10, Yvette: same unnecessary castration. |
| **Felix** | **Popped** — Ch.10, Tamara: removes both after Yvette lets torsion kill them (Kaitlin had twisted them for hours). | **Popped** — Ch.10, Tamara: bilateral orchiectomy. |
| **Trinn** | **Popped** — Ch.11, Tamara: finishes destroying the nut Andrea already wrecked. *(Ch.9 = Andrea pops his **right**; Emma says “you’ve still got **leftie**.” Ch.11 Tamara’s assessment flips which side Andrea took — narration error.)* | **Popped** — Ch.9 Andrea heel-stomps his large **right** nut; Ch.11 Tamara destroys the last healthy remnant with repeated punches. |
| **Desmond** | **Popped** — Ch.11, Fiora: standing full weight on his **left** nut until it cracks and collapses. | **Popped** — Ch.11, Fiora: same balancing stunt on his **right** nut until it flattens into mush. |
| **Nguyen Twins** (each brother) | **Popped** — Ch.11, Yvette: triple green elastrator bands cut blood flow; both twins’ **left** nuts die. | **Popped** — Ch.11, Yvette: same elastrator treatment; both twins’ **right** nuts die. *(Four testicles total across the pair.)* |
| **Judah** | **Popped (offscreen / before Ch.15)** — already a Church half-man with only one nut when captured. | **Damaged** — Ch.15 Andrea kicks; Ch.19 Luna internal audits shred it; Ch.20 Tamara “defragments” the tattered sole gonad back into a smaller dense orb (~30% volume loss) so it can survive more scans. Still attached through Ch.20; outline Ch.21 has Eve pop it. |
| **Matthias** | **N/A** — half-eunuch. | **Popped** — Ch.20: sprints into Izzie’s synchronized BellRinger field during the pre-pop orgy; his sole testicle is pulsed into liquid in seconds. |
| **Isaac** | **Popped** — Ch.17, Kay: bites down and grinds his **healthy left** nut to mush in the First Rite mouth-rite (after promising to take the broken **right**). | **Damaged** — six months of Kay’s “prep” left it liquified/dead but still in the sack; not cleanly popped on-page, but functionally destroyed. |
| **Elijah** | **Popped** — Ch.18, Abby: crushes **both** nuts at once in her fist during the Second Rite handjob (full castration, not halfstration). | **Popped** — Ch.18, Abby: same fist crush; both destroyed together. |
| **Unnamed breeding-stable half-eunuchs (×24)** | **Popped** — Ch.19: each man’s **sole** testicle destroyed in the underground breeding stable (BellRinger pulses plus Izzie, Tamara, and Andrea finishing work). | **N/A** — each entered as a half-eunuch with one testicle only; all twenty-four sole gonads lost on-page in Ch.19. |
| **Unnamed ritual-chamber / bugzapper victims (×43+)** | **Popped** — Ch.20: Izzie’s synchronized BellRinger disc in the pre-pop orgy room. Five stallions lose their sole nuts on the first pulse; **thirty-nine** more half-men lose theirs sprinting into range (Judah’s count); Tamara and Andrea finish **four** survivors found still solid in the entrance piles. Each victim had one testicle only. | **N/A** — half-eunuchs only. |

## Testicles lost and female orgasms

| Chapter | Testicles lost | Female orgasms |
| --- | --- | --- |
| **1** | 1 | 0 |
| **2** | 0 | 0 |
| **3** | 2 | 2 |
| **4** | 0 | 2 |
| **5** | 0 | 0 |
| **6** | 4 | 0 |
| **7** | 1 | 0 |
| **8** | 1 | 2 |
| **9** | 1 | 1 |
| **10** | 4 | 2 |
| **11** | 26+ | 35+ |
| **12** | 0 | 2 |
| **13** | 0 | 0 |
| **14** | 0 | 0 |
| **15** | 2 | 2 |
| **16** | 0 | 0 |
| **17** | 1 | 7+ |
| **18** | 2 | 1 |
| **19** | 24 | 18+ |
| **20** | 48 | 11+ |
| **Total** | 116 | 85+ |

_Per-chapter counts; see [notes.md](./notes.md) for methodology and commentary. Ch.21–22 are outline-only in the published doc and excluded._
