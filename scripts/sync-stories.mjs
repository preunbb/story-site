...
function pruneOrphans(validIds, stories) {
  if (!existsSync(OUT_DIR)) return [];
  const removed = [];
  const noFullMd = new Set();
  for (const s of stories || []) {
    // Password-gated / preview-only stories must not keep a plaintext <id>.md
    // when the preview lives in a separate *-preview.md file.
    if (!s.driveUrl && s.previewRead && s.previewRead.md) {
      const previewPath = resolve(repoRoot, s.previewRead.md);
      const fullPath = join(OUT_DIR, `${s.id}.md`);
      if (previewPath !== fullPath) noFullMd.add(String(s.id));
    }
  }
  for (const f of readdirSync(OUT_DIR)) {
    ...
    // Keep encrypt payloads elsewhere; never treat .enc.json here.
...
async function main() {
  ...
  if (ARGS.prune && !ARGS.only) {
    ...
    pruned = pruneOrphans(validIds, all);
