...
export const BEREAVEMENT_COUNSELING_STORY_ID = 49;
export const BEREAVEMENT_COUNSELING_MD = join(
  repoRoot,
  "dist",
...
export function isBereavementCounselingExport(storyId) {
  return Number(storyId) === BEREAVEMENT_COUNSELING_STORY_ID;
}
 ...
 * Story id 49 (Bereavement Counseling) reads
 * dist/bereavement-counseling/story.md via `npm run sync:bereavement`.
 */
export function readStoryMarkdownForExport(storyId) {
  ...
  if (isBereavementCounselingExport(storyId)) {
    if (!existsSync(BEREAVEMENT_COUNSELING_MD)) {
      throw new Error(
        `Missing ${BEREAVEMENT_COUNSELING_MD}. Run \`npm run sync:bereavement\` first.`,
      );
    }
    return readFileSync(BEREAVEMENT_COUNSELING_MD, "utf8");
  }
